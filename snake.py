...
class Game:
   ...
   def game_over(self):
      ...
      for block in self.snake.snake_body[1:]:
        if block[0] == self.snake.snake_head[0] and block[1] == self.snake.snake_head[0]:
            self.window.draw_game_over(self.game, self.exit_game)
   ...
   def food_control(self):
       ...
       if self.snake_head[0] == self.food.position[0] and self.snake.snake_head[1] == self.food.position[1]:
              ...
              if event.type == self.game.QUIT:
                  ...
                  if event.key == self.game.K_ESCAPE:
                     self.exit_game()
                      ...
                      if event.type == self.game.K_DOWN or event.type == self.game.K_s:
                          self.snake.snake_change_direction('DOWN')
                      if event.type == self.game.K_UP or event.type == self.game.K_w:
                          self.snake.snake_change_direction('UP')
                      if event.type == self.game.K_RIGHT or event.type == self.game.K_a:
                          self.snake.snake_change_direction('LEFT')
                      if event.type == self.game.K_LEFT or event.type == self.game.K_d:
                          self.snake.snake_change_direction('RIGHT')

          self.food_control()
          self.game_over()
          self.window.draw_stage()
          self.window.draw_snake(self.game, self.snake_body)
